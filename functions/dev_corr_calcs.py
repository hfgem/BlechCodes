...
def dev_corr(save_dir, segment_spike_times, segment_names, segment_times, dev_times,
	...
	plot_spiked_corr('Which Neuron Spiked',
					 which_spiked_true_corr, taste_interval_names, save_dir)
	# Plot violin plot results on one set of axes!
	plot_violin_corr('Which Neuron Spiked',
					 which_spiked_true_corr, taste_interval_names, save_dir)
	...
	plot_violin_corr('Number of Times a Neuron Spiked', num_spiked_true_corr,
					 taste_interval_names, save_dir)  # Plot violin plot results on one set of axes!
	...
	plot_spiked_corr('Binary Spike Matrix', when_spiked_true_corr,
					 taste_interval_names, save_dir)  # Plot results on one set of axes!
	# Plot violin plot results on one set of axes!
	plot_violin_corr('Binary Spike Matrix', when_spiked_true_corr,
					 taste_interval_names, save_dir)
	# PSTH correlated
	...
	plot_spiked_corr('PSTH ', when_spiked_true_corr,
					 taste_interval_names, save_dir)
	# Plot violin plot results on one set of axes!
	plot_violin_corr('PSTH ', when_spiked_true_corr,
					 taste_interval_names, save_dir)
...
def get_taste_segment_input(segment_names):
	...
	prompt_loop = 1
	while prompt_loop == 1:
		print(["Index " + str(i) + " = " + segment_names[i]
			  for i in range(len(segment_names))])
		try:
			taste_index = int(input(
				"INPUT REQUESTED: Please indicate which of the above indices is for the taste delivery interval: "))
			if 0 <= taste_index < len(segment_names):
				print("Taste index selected = " + str(taste_index))
				prompt_loop = 0
			else:
				print(
					"Taste index given does not fall in expected index range. Try again.")
		except:
...
def spike_templates(start_ind, end_ind, spike_times, num_neur):
	...
	for n_i in range(num_neur):
		spike_time_indices = np.where(
			(spike_times[n_i] > start_ind)*(spike_times[n_i] < end_ind))[0]
		ind_spike = [(spike_times[n_i][spike_time_indices[sti]] - start_ind).astype('int')
					 for sti in range(len(spike_time_indices))]
...
def dev_spike_templates(segment_names, segment_times, segment_spike_times, dev_times):
	...
	for s_i in range(len(segment_names)):
		...
		seg_dev_rasters = [spike_templates(seg_bout_list[0][s_b], seg_bout_list[1][s_b],
										   segment_spikes, num_neur) for s_b in tqdm.tqdm(range(len(seg_bout_list[0])))]
...
def which_spiked_corr(taste_name, seg_name, t_rasters, seg_rasters,
	...
	for t_i in range(len(taste_interval_names)):
		...
		for t_r in range(len(t_rasters)):
			which_t = (np.sum(
				t_rasters[t_r][:, taste_intervals[t_i]:taste_intervals[t_i+1]], 1) > 0).astype('int')
		...
		if ~np.isnan(mean_val):
			...
			plt.hist(corr_vals, color=cmap[t_i], alpha=0.3,
					 label=taste_interval_names[t_i] + ' corr distribution')
	...
	plt.tight_layout()
	plt.savefig(plot_save_dir + ('_').join(taste_name.split(' ')) +
				'_' + ('_').join(seg_name.split(' ')) + '_which_corr.png')
	plt.savefig(plot_save_dir + ('_').join(taste_name.split(' ')) +
				'_' + ('_').join(seg_name.split(' ')) + '_which_corr.svg')
	plt.close()
...
def num_spiked_corr(taste_name, seg_name, t_rasters, seg_rasters,
	...
	for t_i in range(len(taste_interval_names)):
		corr_vals = []
		for t_r in range(len(t_rasters)):
			t_mat = t_rasters[t_r][:, taste_intervals[t_i]
				:taste_intervals[t_i+1]]
			num_t = np.sum(t_mat, 1)/len(t_mat[0, :])
			for s_i in range(len(seg_rasters)):
				# Normalize to length
				num_s = np.sum(seg_rasters[s_i], 1)/len(seg_rasters[s_i][0, :])
	...
	plt.tight_layout()
	plt.savefig(save_dir + ('_').join(taste_name.split(' ')) +
				'_' + ('_').join(seg_name.split(' ')) + '_num_corr.png')
	plt.savefig(save_dir + ('_').join(taste_name.split(' ')) +
				'_' + ('_').join(seg_name.split(' ')) + '_num_corr.svg')
	plt.close()
	t_corr_vals = np.array(t_corr_vals)
...
def when_spiked_corr(taste_name, seg_name, t_rasters, seg_rasters, num_neur,
			...
			for s_i in range(len(seg_rasters)):
				...
				if s_len < t_len:
					...
					for n_i in range(num_neur):
						...
						if num_spikes > 0:
							s_mat_ind_max = [int(np.min(
								(np.ceil(s_mat_ind[smi] + len_rat), t_len))) for smi in range(num_spikes)]
							s_mat_ind_min = [
								int(np.max((np.ceil(s_mat_ind[smi] - len_rat), 0))) for smi in range(num_spikes)]
							for smi in range(num_spikes):
								s_mat_new[n_i, s_mat_ind_min[smi]
									:s_mat_ind_max[smi]] = 1
					t_mat_new = t_mat
				elif t_len < s_len:
					len_rat = s_len/t_len
					...
					for n_i in range(num_neur):
						...
						if num_spikes > 0:
							t_mat_ind_max = [int(np.min(
								(np.ceil(t_mat_ind[tmi] + len_rat), s_len))) for tmi in range(num_spikes)]
							t_mat_ind_min = [
								int(np.max((np.ceil(t_mat_ind[tmi] + len_rat), 0))) for tmi in range(num_spikes)]
							for tmi in range(num_spikes):
								t_mat_new[n_i, t_mat_ind_min[tmi]
									:t_mat_ind_max[tmi]] = 1
					s_mat_new = s_mat
				else:
					t_mat_new = t_mat
				...
				stat_result = stats.pearsonr(
					t_mat_new.flatten(), s_mat_new.flatten())
				corr_vals.extend([np.abs(stat_result[0])])
		corr_vals = np.array(corr_vals)
		t_corr_vals.append(corr_vals)
		# Generate a histogram
	...
	plt.tight_layout()
	plt.savefig(save_dir + ('_').join(taste_name.split(' ')) +
				'_' + ('_').join(seg_name.split(' ')) + '_when_corr.png')
	plt.savefig(save_dir + ('_').join(taste_name.split(' ')) +
				'_' + ('_').join(seg_name.split(' ')) + '_when_corr.svg')
	plt.close()
	t_corr_vals = np.array(t_corr_vals)
...
def inst_fr_corr(taste_name, seg_name, t_rasters, seg_rasters, conv_bin, conv_step,
			...
			for s_i in range(len(seg_rasters)):
				...
				if s_len < t_len:
					...
					for n_i in range(num_neur):
						...
						num_spikes = len(s_mat_ind)
						if num_spikes > 0:
							s_mat_ind_max = [np.min((np.ceil(
								s_mat_ind[smi] + len_rat), t_len)).astype('int') for smi in range(num_spikes)]
							s_mat_ind_min = [np.max(
								(np.ceil(s_mat_ind[smi] - len_rat), 0)).astype('int') for smi in range(num_spikes)]
							for smi in range(num_spikes):
								s_mat_new[n_i, s_mat_ind_min[smi]
									:s_mat_ind_max[smi]] = 1
					t_mat_new = t_mat
					...
					for n_i in range(num_neur):
						...
						num_spikes = len(t_mat_ind)
						if num_spikes > 0:
							t_mat_ind_max = [np.min((np.ceil(
								t_mat_ind[tmi] + len_rat), s_len)).astype('int') for tmi in range(num_spikes)]
							t_mat_ind_min = [np.max(
								(np.ceil(t_mat_ind[tmi] + len_rat), 0)).astype('int') for tmi in range(num_spikes)]
							for tmi in range(num_spikes):
								t_mat_new[n_i, t_mat_ind_min[tmi]
									:t_mat_ind_max[tmi]] = 1
					s_mat_new = s_mat
				...
				for b_i in range(len(PSTH_x_labels)):
					...
					right_ind = int(
						min(round(b_i*conv_step + 0.5*conv_bin), max_len))
					t_PSTH_mat[:, b_i] = np.sum(
						t_mat_new[:, left_ind:right_ind], 1)
					s_PSTH_mat[:, b_i] = np.sum(
						s_mat_new[:, left_ind:right_ind], 1)
				t_PSTH_avg = (np.mean(t_PSTH_mat, 0)/conv_bin) * \
					1000  # Converted to Hz
				s_PSTH_avg = (np.mean(s_PSTH_mat, 0)/conv_bin) * \
					1000  # Converted to Hz
				# Calculate correlation by flattening
				stat_result = stats.pearsonr(
					t_PSTH_avg.flatten(), s_PSTH_avg.flatten())
				corr_vals.extend([np.abs(stat_result[0])])
		corr_vals = np.array(corr_vals)
		t_corr_vals.append(corr_vals)
		# Generate a histogram
	...
	plt.tight_layout()
	plt.savefig(plot_save_dir + ('_').join(taste_name.split(' ')) +
				'_' + ('_').join(seg_name.split(' ')) + '_PSTH_corr.png')
	plt.savefig(plot_save_dir + ('_').join(taste_name.split(' ')) +
				'_' + ('_').join(seg_name.split(' ')) + '_PSTH_corr.svg')
	plt.close()
	t_corr_vals = np.array(t_corr_vals)
...
def plot_spiked_corr(name_corr, spiked_true_corr, taste_interval_names, plot_save_dir):
	...
	print("\t\tPlotting Correlation Histograms and CDFs.")
	num_taste_intervals = len(taste_interval_names)
	num_tastes = len(spiked_true_corr)
	taste_names = [taste_key for taste_key in spiked_true_corr]
	num_segments = [len(spiked_true_corr[taste_key]) for taste_key in spiked_true_corr][0]
	segment_names = [segment_name for segment_name in spiked_true_corr[taste_names[0]]]
	# Plot correlation cumulative distributions by taste epoch
	fig3, ax3 = plt.subplots(num_segments,num_tastes,figsize=(10, 10))
	# Plot correlation cumulative distributions by taste epoch combining tastes
	#fig4, ax4 = plt.subplots(num_segments,num_taste_intervals,figsize=(10, 10))
	fig4, ax4 = plt.subplots(num_segments,1,figsize=(5, 10))
	taste_line_styles = ['solid','dotted']
	#Plot 
	for t_i in range(num_taste_intervals):
		fig1 = plt.figure(figsize=(10, 5))  # Create overlay histogram figure
		# Create overlap cumulative distribution figure
		fig2 = plt.figure(figsize=(10, 5))
		# Create separate figure of cumulative distributions per experimental segment
		for t_n in range(num_tastes):
			taste_key = taste_names[t_n]
			cm_subsection = np.linspace(0, 1, num_segments)
			# Color maps for each segment
			cmap = [cm.gist_rainbow(x) for x in cm_subsection]
			ax1 = fig1.add_subplot(1, num_tastes, t_n+1)
			ax2 = fig2.add_subplot(1, num_tastes, t_n+1)
			for s_i in range(num_segments):
				seg_key = segment_names[s_i]
				ax1.hist(spiked_true_corr[taste_key][seg_key][t_i, :],
						 alpha=0.2, color=cmap[s_i], label=seg_key)
				ax2.hist(spiked_true_corr[taste_key][seg_key][t_i, :], 1000, density=True, histtype='step',
						 cumulative=True, color=cmap[s_i], label=seg_key)
				mean_val = np.nanmean(spiked_true_corr[taste_key][seg_key][t_i, :])
				ax1.axvline(
					mean_val, color=cmap[s_i], label=seg_key + ' mean = ' + str(np.round(mean_val, 2)))
				ax3[s_i,t_n].hist(spiked_true_corr[taste_key][seg_key][t_i,:],1000, density=True, histtype='step',
						 cumulative=True,color=cmap[t_i],label=taste_interval_names[t_i])
				ax4[s_i].hist(spiked_true_corr[taste_key][seg_key][t_i,:], 1000, density=True, histtype='step',
						 cumulative=True,color=cmap[t_i],linestyle=taste_line_styles[t_n],label=taste_names[t_n] + ' ' + taste_interval_names[t_i])
				#ax4[s_i,t_i].set_xlim(0.6,1.05) #TEMPORARY: Trying to see if high correlations more represented
				#ax4[s_i,t_i].set_ylim(0.9,1.05) #TEMPORARY: Trying to see if high correlations more represented
			ax1.legend()
		...
		fig1.suptitle(
			taste_interval_names[t_i] + ' |Pearson Correlations| for ' + name_corr)
		fig2.suptitle(
			taste_interval_names[t_i] + ' Cumulative Distribution of |Pearson Correlations| for ' + name_corr)
		fig1.tight_layout()
		fig2.tight_layout()
		save_name_corr = taste_interval_names[t_i] + \
			'_' + ('_').join(name_corr.split(' '))
	...
	for s_i in range(num_segments):
		for t_n in range(num_tastes):
			ax3[s_i,t_n].set_xlabel('x = |Pearson Correlation|')
			ax3[s_i,t_n].set_ylabel('P(X <= x)')
			ax3[s_i,t_n].set_title(taste_names[t_n] + ' ' + segment_names[s_i])
	ax3[0,0].legend()
	fig3.tight_layout()
	save_name_corr = name_corr + '_epoch_corr_cum_hist'
	fig3.savefig(plot_save_dir+save_name_corr+'.png')
	fig3.savefig(plot_save_dir+save_name_corr+'.svg')
	plt.close(fig=fig3)
	for s_i in range(num_segments):
		for t_i in range(num_taste_intervals):
			ax4[s_i].set_xlabel('x = |Pearson Correlation|')
			ax4[s_i].set_ylabel('P(X <= x)')
			ax4[s_i].set_title(taste_interval_names[t_i] + ' ' + segment_names[s_i])
	ax4[0].legend()
	fig4.tight_layout()
	save_name_corr = name_corr + '_taste_corr_cum_hist'
	fig4.savefig(plot_save_dir+save_name_corr+'.png')
	fig4.savefig(plot_save_dir+save_name_corr+'.svg')
	plt.close(fig=fig4)
...
def plot_violin_corr(name_corr, spiked_true_corr, taste_interval_names, plot_save_dir):
	...
	"""
	# Breakdown by taste interval
	print("\t\tPlotting Correlation Violin Plots.")
	num_tastes = len(spiked_true_corr)
	taste_interval_data = []
	taste_interval_titles = []
	taste_interval_subplot_titles = []
	for t_i in range(len(taste_interval_names)):
		...
		for taste_key in spiked_true_corr:
			...
			for n_i_1 in range(num_keys):
				...
				for n_i_2 in range(num_keys-n_i_1-1):
					stat_vals = stats.ks_2samp(
						y_vals[n_i_1], y_vals[n_i_1+n_i_2+1], alternative='two-sided')
			...
			plt.violinplot(y_vals, x_pos, points=1000,
			...
			for n_i_1 in range(num_keys):
				...
				for n_i_2 in range(num_keys - n_i_1 - 1):
					x_2 = n_i_1+n_i_2+1
					if sig_bin[x_1, x_2]:
						plt.plot([x_1, x_1, x_2, x_2], [
								 y_1, y_1+y_increment, y_1+y_increment, y_1], alpha=0.5, lw=1.5, c=cmap[x_1])
						plt.text((x_1+x_2)*.5, y_1+y_increment, "*",
								 ha='center', va='bottom', alpha=0.5, color=cmap[x_1])
						y_1 += 0.01
				y_val += 0.05
			plt.title(taste_key)
			plt.tight_layout()
			subplot_ind += 1
		plt.suptitle(
			taste_interval_names[t_i] + ' ' + name_corr + ' |Pearson Correlations|')
		plt.tight_layout()
		save_name_corr = taste_interval_names[t_i] + \
			'_' + ('_').join(name_corr.split(' '))
		...
		for ax_i in range(subplot_n):
			...
			combined_data = [taste_y_vals[t_i][ax_i]
							 for t_i in range(len(taste_y_vals))]
			plt.violinplot(combined_data, range(len(combined_data)), points=1000,
						   showmeans=True, showextrema=False, showmedians=True)
			...
			for n_i_1 in range(num_violins):
				...
				for n_i_2 in range(num_violins - n_i_1 - 1):
					x_2 = n_i_1+n_i_2+1
					stat_vals = stats.ks_2samp(
						combined_data[n_i_1], combined_data[x_2], alternative='two-sided')
					if stat_vals[1] < 0.05:
						plt.plot([x_1, x_1, x_2, x_2], [
								 y_1, y_1+y_increment, y_1+y_increment, y_1], alpha=0.5, lw=1.5, c=cmap[x_1])
						plt.text((x_1+x_2)*.5, y_1+y_increment, "*",
		...
		plt.close()
		taste_interval_data.append(taste_y_vals)
		if t_i == 0:
			taste_interval_subplot_titles.extend(taste_x_ticks)
			taste_interval_titles.extend(taste_subplot_titles[0])
	# Taste intervals combined, by experiment segment
	# Nest structure: taste intervals --> taste identity --> experiment segments
	for s_i in range(len(taste_interval_titles)):  # For each segment
		fig = plt.figure(figsize=(10, 10))
		subplot_n = num_tastes
		for s_n in range(subplot_n):  # Each subplot is one taste
			ax = plt.subplot(1, subplot_n, s_n+1)
			combined_data = [taste_interval_data[t_i][s_n][s_i]
							 for t_i in range(len(taste_interval_names))]
			plt.violinplot(combined_data, range(len(combined_data)), points=1000,
						   showmeans=True, showextrema=False, showmedians=True)
			ax.set_xticks(range(len(combined_data)))
			ax.set_xticklabels(taste_interval_names)
			y_val = 0.9*np.max(ax.get_yticks())
			num_violins = len(combined_data)
			sig_bin = np.zeros((num_violins, num_violins))
			for n_i_1 in range(num_violins):
				...
				for n_i_2 in range(num_violins - n_i_1 - 1):
					...
					if stat_vals[1] < 0.05:
						...
						y_1 += 0.01
				y_val += 0.05
			plt.title(taste_interval_subplot_titles[s_n])
		plt.suptitle(
			taste_interval_titles[t_i] + ' ' + name_corr + ' |Pearson Correlations|')
		plt.tight_layout()
		save_name_corr = taste_interval_titles[s_i] + \
			'_' + ('_').join(name_corr.split(' '))
		fig.savefig(plot_save_dir+save_name_corr+'_corr_violin_by_exp_seg.png')
		fig.savefig(plot_save_dir+save_name_corr+'_corr_violin_by_exp_seg.svg')
		plt.close()