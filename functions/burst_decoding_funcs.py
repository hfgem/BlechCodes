    ...
    for e_i in epochs_to_analyze:  # By epoch conduct decoding
        ...
        for t_i in range(num_tastes):
            for d_i in range(max_num_deliv):
                if d_i >= trial_start_ind:
                    full_data.extend(
                        tastant_fr_dist[t_i][d_i-trial_start_ind][e_i].T)
        gm = gmm(n_components=1,  n_init=10).fit(full_data)
        fit_all_neur = gm
        ...
        seg_decode_percents_full = np.zeros(
            (num_tastes, len(segments_to_analyze)))
        ...
        for s_ind, s_i in enumerate(segments_to_analyze):
            ...
            try:
                burst_decode_array = np.load(
                    epoch_save_dir + 'segment_' + str(s_i) + '_burst_decode.npy')
                ...
                try:
                    ...
                    for dev_i in range(num_devs):
                        dev_fr_i = np.load(
                            dev_fr_save_dir + 'dev_' + str(dev_i) + '.npy')
                    ...
                    for dev_i in range(num_devs):
                        ...
                        dev_fr_i = np.sum(
                            segment_spike_times_s_i_bin[:, dev_start_i:dev_end_i], 1)/(dev_len*(1/1000))
                        np.save(dev_fr_save_dir + 'dev_' +
                                str(dev_i) + '.npy', dev_fr_i)
                        dev_fr.append(dev_fr_i)
                ...
                inputs = zip(dev_fr, itertools.repeat(num_tastes),
                             itertools.repeat(
                                 num_neur), itertools.repeat(x_vals),
                             itertools.repeat(fit_tastant_neur), itertools.repeat(
                                 fit_all_neur),
                             itertools.repeat(p_taste), itertools.repeat(taste_select_neur))
                tic = time.time()
                pool = Pool(4)
                burst_decode_prob = pool.map(
                    dp.segment_burst_decode_dependent_parallelized, inputs)
                pool.close()
                toc = time.time()
                ...
                np.save(epoch_save_dir + 'segment_' + str(s_i) +
                        '_burst_decode.npy', burst_decode_array)
            ...
            for dev_i in range(num_devs):
                dev_start_i = seg_deviation_times[0, dev_i]
                dev_end_i = seg_deviation_times[1, dev_i]
                seg_burst_timing_array[:, dev_start_i:dev_end_i] = np.expand_dims(
                    burst_decode_array[:, dev_i], 1)
            np.save(epoch_save_dir + 'segment_' + str(s_i) +
                    '_burst_decode_full_time.npy', seg_burst_timing_array)

            seg_burst_timing_taste_ind = np.argmax(seg_burst_timing_array, 0)
            # Updated decoding based on threshold
            seg_burst_timing_taste_bin = np.zeros(
                np.shape(seg_burst_timing_array))
            for t_i in range(num_tastes):
            ...
            f1 = plt.figure()
            plt.plot(seg_deviation_times[0, :] /
                     1000/60, seg_decode_epoch_prob_nonan.T)
            for t_i in range(num_tastes):
                plt.fill_between(
                    seg_deviation_times[0, :]/1000/60, seg_decode_epoch_taste_bin[t_i, :], alpha=0.2)
            plt.legend(dig_in_names, loc='right')
            plt.ylabel('Decoding Fraction')
            plt.xlabel('Time (min)')
            plt.title('Segment ' + str(s_i))
            f1.savefig(seg_save_dir + 'segment_' +
                       str(s_i) + '_dev_decode.png')
            f1.savefig(seg_save_dir + 'segment_' +
                       str(s_i) + '_dev_decode.svg')
            plt.close(f1)
            f1 = plt.figure()
            ...
            plt.title('Segment ' + str(s_i))
            f1.savefig(seg_save_dir + 'segment_' +
                       str(s_i) + '_full_time_dev_decode.png')
            f1.savefig(seg_save_dir + 'segment_' +
                       str(s_i) + '_full_time_dev_decode.svg')
            plt.close(f1)
            # Imshow
            f2 = plt.figure()
            ...
            plt.title('Segment ' + str(s_i))
            f2.savefig(seg_save_dir + 'segment_' +
                       str(s_i) + '_dev_decode_im.png')
            f2.savefig(seg_save_dir + 'segment_' +
                       str(s_i) + '_dev_decode_im.svg')
            plt.close(f2)
            f2 = plt.figure()
            plt.imshow(seg_burst_timing_array,
                       aspect='auto', interpolation='none')
            y_ticks = np.arange(len(dig_in_names))
            plt.yticks(y_ticks, dig_in_names)
            plt.ylabel('Decoding Fraction')
            plt.xlabel('Time (min)')
            plt.title('Segment ' + str(s_i))
            f2.savefig(seg_save_dir + 'segment_' + str(s_i) +
                       '_full_time_dev_decode_im.png')
            f2.savefig(seg_save_dir + 'segment_' + str(s_i) +
                       '_full_time_dev_decode_im.svg')
            plt.close(f2)
            # Fraction of occurrences
            f3 = plt.figure()
            dev_decode_pie = np.sum(
                seg_decode_epoch_taste_bin, 1)/np.sum(seg_decode_epoch_taste_bin)
            seg_decode_percents[:, s_ind] = dev_decode_pie.T
            plt.pie(dev_decode_pie, labels=dig_in_names,
                    autopct='%1.1f%%', pctdistance=1.5)
            plt.title('Segment ' + str(s_i))
            f3.savefig(seg_save_dir + 'segment_' +
                       str(s_i) + '_dev_decode_pie.png')
            f3.savefig(seg_save_dir + 'segment_' +
                       str(s_i) + '_dev_decode_pie.svg')
            plt.close(f3)
            f3 = plt.figure()
            full_time_dev_decode_pie = np.sum(
                seg_burst_timing_taste_bin, 1)/np.sum(seg_burst_timing_taste_bin)
            seg_decode_percents_full[:, s_ind] = full_time_dev_decode_pie.T
            plt.pie(full_time_dev_decode_pie, labels=dig_in_names,
                    autopct='%1.1f%%', pctdistance=1.5)
            plt.title('Segment ' + str(s_i))
            f3.savefig(seg_save_dir + 'segment_' + str(s_i) +
                       '_full_time_dev_decode_pie.png')
            f3.savefig(seg_save_dir + 'segment_' + str(s_i) +
                       '_full_time_dev_decode_pie.svg')
            plt.close(f3)

        f = plt.figure()
        plt.plot(seg_decode_percents.T, label=dig_in_names)
        plt.xticks(np.arange(len(segments_to_analyze)),
                   labels=np.array(segment_names)[segments_to_analyze])
        plt.legend()
        ...
        plt.close(f)
        f, ax = plt.subplots(nrows=1, ncols=num_tastes,
                             figsize=(num_tastes*3, 3))
        for t_i in range(num_tastes):
            ax[t_i].plot(seg_decode_percents_full[t_i, :])
            ax[t_i].set_xticks(np.arange(len(segments_to_analyze)), labels=np.array(
                segment_names)[segments_to_analyze], rotation=45)
    ...
    for e_i in epochs_to_analyze:  # By epoch conduct decoding
        ...
        for t_i in range(num_tastes):
            ...
            for d_i in range(max_num_deliv):
                if d_i >= trial_start_ind:
                    full_data.extend(
                        tastant_fr_dist_z[t_i][d_i-trial_start_ind][e_i])
            gm = gmm(n_components=1, n_init=10).fit(full_data)
            fit_tastant_neur[t_i] = gm
...
def plot_decoded(fr_dist, num_tastes, num_neur, num_cp, segment_spike_times, tastant_spike_times,
    ...
    epoch_seg_taste_percents_neur_cut = np.zeros(
        (num_cp, num_segments, num_tastes))
    epoch_seg_taste_percents_best = np.zeros(
        (num_cp, num_segments, num_tastes))
    half_bin_z_dt = np.floor(bin_dt/2).astype('int')
        ...
        for s_i in tqdm.tqdm(segments_to_analyze):
            try:
                seg_decode_epoch_prob = np.load(
                    epoch_decode_save_dir + 'segment_' + str(s_i) + '_burst_decode_full_time.npy')
            except:
                ...
                for d_i in range(len(taste_spike_times)):
                    if d_i >= trial_start_ind:
                        ...
                        taste_cp_times[d_i-trial_start_ind, :] = np.concatenate(
                            (np.zeros(1), np.cumsum(np.diff(pop_taste_cp_times[d_i, :])))).astype('int')
            ...
            for t_i in range(num_tastes):
                taste_decode_save_dir = seg_decode_save_dir + \
                    dig_in_names[t_i] + '_events/'
                ...
                epoch_seg_taste_percents[e_i, s_i, t_i] = (
                ...
                epoch_seg_taste_percents_neur_cut[e_i, s_i, t_i] = (
                ...
                all_taste_event_fr_vecs_neur_cut.append(
                    np.array(decoded_fr_vecs)[decode_ind])
                all_taste_event_fr_vecs_z_neur_cut.append(
                    np.array(decoded_z_fr_vecs)[decode_ind])
                ...
                for db in best_across_metrics:
                    ...
                    decoded_bin[s_db:e_db] = 1
                # Grab overall percentages
                epoch_seg_taste_percents_best[e_i, s_i, t_i] = (
                    np.sum(decoded_bin)/len(decoded_bin))*100
                # Re-calculate start and end times of the decoded intervals
                ...
                if len(decode_plot_ind) > 0:
                    for nd_i in decode_plot_ind:
                        ...
                        ax[0, 1].axvline(d_start-d_plot_start,
                                         color='k', alpha=0.5)
                        ax[0, 1].axvline(d_end-d_plot_start,
                                         color='k', alpha=0.5)
                        ...
                        plt.colorbar(img, ax=ax[3, 0], location='bottom', orientation='horizontal',
                                     label='Firing Rate (Hz)', panchor=(0.9, 0.5))
                        ...
                        plt.tight_layout()
                        # Save Figure
                        f.savefig(taste_decode_save_dir +
                                  'event_' + str(nd_i) + '.png')
                        f.savefig(taste_decode_save_dir +
                                  'event_' + str(nd_i) + '.svg')
                        plt.close(f)
...
#
    ...
    for e_i in epochs_to_analyze:
        for t_i in range(num_tastes):
            ...
            plt.plot(segments_to_analyze,
                     (epoch_seg_taste_percents[e_i, segments_to_analyze, t_i]).flatten())
    ...
    for e_i in epochs_to_analyze:
        for t_i in range(num_tastes):
            plt.subplot(num_cp, num_tastes, plot_ind)
            plt.plot(segments_to_analyze,
                     (epoch_seg_taste_percents_neur_cut[e_i, segments_to_analyze, t_i]).flatten())
            seg_labels = [segment_names[a] for a in segments_to_analyze]
            plt.xticks(segments_to_analyze, labels=seg_labels, rotation=-45)
            if t_i == 0:
                plt.ylabel('Epoch ' + str(e_i))
    ...
    for e_i in epochs_to_analyze:
        for t_i in range(num_tastes):
            plt.subplot(num_cp, num_tastes, plot_ind)
            plt.plot(segments_to_analyze,
                     (epoch_seg_taste_percents_best[e_i, segments_to_analyze, t_i]).flatten())
            seg_labels = [segment_names[a] for a in segments_to_analyze]
            plt.xticks(segments_to_analyze, labels=seg_labels, rotation=-45)
            if t_i == 0:
                plt.ylabel('Epoch ' + str(e_i))
...
def plot_decoded_func_p(fr_dist, num_tastes, num_neur, num_cp, segment_spike_times, tastant_spike_times,
    ...
    f, ax = plt.subplots(num_cp, len(segments_to_analyze) + 1,
                         figsize=(10, 10), width_ratios=sum_width_ratios)
    gs = ax[0, -1].get_gridspec()
    for e_i in epochs_to_analyze:
        for s_i in segments_to_analyze:
            ax[e_i, s_i].set_ylim([0, max_decoding_percent+10])
            for t_i in range(num_tastes):
                ax[e_i, s_i].plot(prob_cutoffs, (epoch_seg_taste_percents[:, e_i, s_i, t_i]).flatten(
...
def plot_decoded_func_n(fr_dist, num_tastes, num_neur, num_cp, segment_spike_times, tastant_spike_times,
    ...
    sum_width_ratios = np.concatenate(
        (10*np.ones(len(segments_to_analyze)), np.ones(1)))
    max_decoding_percent = np.max(epoch_seg_taste_percents)
    f, ax = plt.subplots(num_cp, num_segments + 1,
                         figsize=(10, 10), width_ratios=sum_width_ratios)
    gs = ax[0, -1].get_gridspec()
    for e_i in epochs_to_analyze:
        for s_i in segments_to_analyze:
            ax[e_i, s_i].set_ylim([0, max_decoding_percent+10])
            for t_i in range(num_tastes):
                ax[e_i, s_i].plot(neur_cutoffs, (epoch_seg_taste_percents[:, e_i, s_i, t_i]).flatten(
                ), color=taste_colors[t_i, :], alpha=0.8)
            if s_i == 0:
                ax[e_i, s_i].set_ylabel('Epoch ' + str(e_i))
            if e_i == 0:
