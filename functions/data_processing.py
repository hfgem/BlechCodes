...
def file_names():
    ...
    currdir = os.getcwd()
    datadir = fd.askdirectory(parent=root, initialdir=currdir,
...
def dig_ins(dat_files_list):
    ...
    if len(dig_ins) > 0:
        ...
        dig_ins = [
            name for name in dat_files_list if name.startswith('digital')]
        if len(dig_ins) > 0:
            dig_in_names = list()
            dig_in_name_loop = 1
            i = 0
...
def get_experiment_components(sampling_rate, dig_ins=None, dig_in_ind_range=np.zeros((2)), len_rec=0):
    ...
    while segments_loop == 1:
        try:
            ...
            print(
                "For example, a pre-tastant delivery interval, a taste delivery interval, etc...")
            num_segments = int(
                input("How many segments comprise your experiment? Enter an integer: "))
            segments_loop = 0
        except:
            print("Incorrect entry, please enter integer.")
    ...
    remaining_indices = np.setdiff1d(
        np.arange(num_segments), np.array([0, taste_ind, num_segments-1]))
    for i in remaining_indices:
        int_len_loop = 1
        while int_len_loop == 1:
            try:
                int_len = int(input("\n INPUT REQUESTED: How long was the " +
                              segment_names[i] + " interval? (in rounded minutes) \n"))
                int_len_loop = 0
            except:
                print("Error: Value must be an integer. Try again.")
        # Converted to seconds and then samples / second
        int_len_samples = sampling_rate*60*int_len
