...
def FR_dev_calcs(fig_save_dir, segment_names, segment_times, segment_spike_times,
    ...
    local_bin_size, deviation_bin_size, dev_thresh, std_cutoff, partic_neur_cutoff, any_change = import_settings(
        hf5_dir, local_bin_size, deviation_bin_size, dev_thresh, std_cutoff, partic_neur_cutoff)
    ...
    segment_devs, segment_dev_frac_ind = dev_calcs(hf5_dir, num_neur, num_segments, segment_names, segment_times,
                                                   segment_spike_times, deviation_bin_size, local_bin_size, std_cutoff, partic_neur_cutoff, any_change)

    # Calculate deviation bouts
    segment_bouts, segment_bout_lengths, segment_ibis = deviation_bout_ibi_calc(
        hf5_dir, num_segments, segment_names, segment_times, num_neur, dev_save_dir, segment_devs, deviation_bin_size, any_change)

    # Calculate mean/std of bout data
    mean_segment_bout_lengths, std_segment_bout_lengths, mean_segment_ibis, std_segment_ibis, num_dev_per_seg, dev_per_seg_freq = mean_std_bout_ibi_calc(
        num_segments, num_neur, segment_names, segment_times, dev_save_dir, segment_bout_lengths, segment_ibis)

    # Calculate null distribution deviation bins
    null_segment_dev_counts, null_segment_dev_ibis, null_segment_dev_bout_len = null_dev_calc(
        hf5_dir, num_segments, num_neur, segment_names, segment_times, num_null_sets, segment_spike_times, deviation_bin_size, local_bin_size, std_cutoff, dev_thresh, partic_neur_cutoff)
...
def import_settings(hf5_dir, local_bin_size, deviation_bin_size, dev_thresh, 
					std_cutoff, partic_neur_cutoff):
    try:
        ...
        local_bin_size, change = keep_func(
            'Local bin size', local_bin_size_import, local_bin_size)
        deviation_bin_size, change1 = keep_func(
            'Deviation bin size', deviation_bin_size_import, deviation_bin_size)
        dev_thresh, change2 = keep_func(
            'Deviation threshold', dev_thresh_import, dev_thresh)
        std_cutoff, change3 = keep_func(
            'Std. cutoff', std_cutoff_import, std_cutoff)
        partic_neur_cutoff, change4 = keep_func(
            'Frac. participating neurons', partic_neur_cutoff_import, partic_neur_cutoff)
