...
def calc_cp_bayes(tastant_spike_times,cp_bin,num_cp,start_dig_in_times,
	...
	for t_i in range(num_tastes):
		...
		neur_deliv_cp = np.zeros((num_deliv,num_neur,num_cp))
		...
		for n_i in tqdm.tqdm(range(num_neur)):
			...
			for d_i in range(num_deliv):
				...
				for time_i in np.arange(cp_bin,bin_length-cp_bin):
					N_1 = np.sum(deliv_bin[:time_i]) #np.sum(deliv_bin[time_i-cp_bin:time_i])
					N_2 = np.sum(deliv_bin[time_i:])#np.sum(deliv_bin[time_i:time_i+cp_bin])
				...
				best_peak_inds = np.zeros(num_cp)
				found_inds = peak_inds[np.sort(ordered_peak_ind[0:num_cp])]
				if len(found_inds) < num_cp:
					diff_len_cp = num_cp - len(found_inds)
					best_peak_inds[0:len(found_inds)] = found_inds
					best_peak_inds[len(found_inds):] = (bin_length-1)*np.ones(diff_len_cp)
				else:
					best_peak_inds[:] = found_inds
			...
			#_____Plot changepoints across deliveries_____
			plot_cp_rasters(neur_deliv_st,neur_cp_likelihood_list,before_taste,
				   dig_in_names[t_i],n_i,num_deliv,num_cp,taste_cp_save_dir)
			for key in neur_cp_likelihood.keys():
...
def plot_cp_rasters(neur_deliv_st,neur_cp_likelihood_list,before_taste,dig_in_name,n_i,num_deliv,num_cp,taste_cp_save_dir):
	#Delivery aligned
