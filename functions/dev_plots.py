...
def plot_deviations(dev_save_dir, num_neur, segment_names, segment_times, dev_thresh,
    ...
    deviation_times(num_segments, num_neur, segment_names,
                    dev_save_dir, segment_devs, dev_thresh)

    # Next plot bout lengths and inter-bout intervals and trends across segments
    segment_bout_stats_plots(num_segments, segment_names,
                             segment_bout_lengths, segment_ibis, dev_save_dir)
...
def deviation_times(num_segments, num_neur, segment_names, dev_save_dir, segment_devs, dev_thresh):
    ...
    for i in tqdm.tqdm(range(num_segments)):
        ...
        fig_i = plt.figure(
            figsize=((max(dev_times)-min(dev_times))*(1/60), 10))
...
def segment_bout_stats_plots(num_segments, segment_names, segment_bout_lengths, 
							 segment_ibis, dev_save_dir):
    ...
    plt.subplot(1, 2, 1)
    plt.plot(segment_names, mean_segment_len,
             color='k', label='Mean Across Neurons')
    plt.plot(segment_names, mean_segment_len + std_segment_len,
             alpha=0.25, color='k', linestyle='-', label='Mean Across Neurons')
    plt.plot(segment_names, mean_segment_len - std_segment_len,
             alpha=0.25, color='k', linestyle='-', label='Mean Across Neurons')
    plt.legend()
    ...
    plt.subplot(1, 2, 2)
    plt.plot(segment_names, mean_segment_ibis,
             color='k', label='Mean Across Neurons')
    plt.plot(segment_names, mean_segment_ibis + std_segment_ibis,
             alpha=0.25, color='k', linestyle='-', label='Mean Across Neurons')
    plt.plot(segment_names, mean_segment_ibis - std_segment_ibis,
             alpha=0.25, color='k', linestyle='-', label='Mean Across Neurons')
    plt.legend()
...
def dev_bin_plots(fig_save_dir, segment_names, segment_times,
    ...
    for s_i in tqdm.tqdm(range(num_segments)):
        ...
        segment_spikes = [np.array(segment_spike_times[s_i][n_i])
                          for n_i in range(num_neur)]
        min_seg_time = segment_times[s_i]
        max_seg_time = segment_times[s_i+1]
        for d_i in range(len(segment_dev_start_times)):
            min_time = max(
                segment_dev_start_times[d_i] - half_local_bin_dt, min_seg_time)
            max_time = min(
                segment_dev_end_times[d_i] + half_local_bin_dt, max_seg_time)
            s_t = []
            for n_i in range(num_neur):
                try:
                    s_t.append(list(segment_spikes[n_i][np.where(
                        (segment_spikes[n_i] >= min_time)*(segment_spikes[n_i] <= max_time))[0]]))
                except:
                    print(segment_spikes[n_i])
            s_t_time = [list(np.array(s_t[i])*(1/1000))
                        for i in range(len(s_t))]  # In seconds
            # Plot segment deviation raster
            plt.figure(figsize=(10, num_neur))
            plt.xlabel('Time (s)')
            plt.ylabel('Neuron Index')
            plt.axvline(
                segment_dev_start_times[d_i]*(1/1000), color='r', alpha=0.4)
            plt.axvline(segment_dev_end_times[d_i]
                        * (1/1000), color='r', alpha=0.4)
...
def null_v_true_dev_plots(dev_save_dir, segment_names, segment_bouts, 
						  segment_bout_lengths, segment_ibis, num_null_sets, 
						  null_segment_dev_counts, null_segment_dev_ibis, 
						  null_segment_dev_bout_len):
    """This function plots histograms of null distribution values and 95th
	percentile cutoffs against true deviation values.
    INPUTS:
            - fig_save_dir
            - segment_names
            - segment_bouts
    ...
    for s_i in range(num_segments):
        print("\tPlotting null distributions for segment " +
              segment_names[s_i])
        # Create Save Directory
        seg_dev_save_dir = dev_save_dir + \
            ('_').join(segment_names[s_i].split(' ')) + '/'
        ...
        plt.subplot(1, 2, 1)
        plt.hist(seg_null_dev_bout_lens_flat, bins=20,
                 alpha=0.5, color='blue', label='Null Data')
        plt.axvline(np.mean(seg_true_dev_bout_lens),
                    color='orange', label='Mean of True Data')
        plt.legend()
        plt.title(im_name + ' Null Distribution')
        plt.xlabel('Deviation Bout Length (s)')
        plt.ylabel('Number of Instances')
        plt.subplot(1, 2, 2)
        plt.hist(seg_true_dev_bout_lens, bins=20, alpha=0.5,
                 color='orange', label='True Data')
        plt.axvline(np.mean(seg_true_dev_bout_lens),
                    color='orange', label='Mean of True Data')
        plt.legend()
        ...
        plt.tight_layout()
        save_name = ('_').join(
            segment_names[s_i].split(' ')) + '_null_v_true_lens'
        ...
        im_name = (' ').join(segment_names[s_i].split('_'))
        plt.subplot(1, 2, 1)
        plt.hist(seg_null_dev_bout_ibis_flat, bins=20,
                 alpha=0.5, color='blue', label='Null Data')
        plt.axvline(np.mean(seg_true_dev_ibis),
                    color='orange', label='Mean of True Data')
        plt.legend()
        plt.title(im_name + ' Null Distribution')
        ...
        plt.ylabel('Number of Instances')
        plt.subplot(1, 2, 2)
        plt.hist(seg_true_dev_ibis, bins=20, alpha=0.5,
                 color='orange', label='True Data')
        plt.axvline(np.mean(seg_true_dev_ibis),
                    color='orange', label='Mean of True Data')
        plt.legend()
        ...
        plt.tight_layout()
        save_name = ('_').join(
            segment_names[s_i].split(' ')) + '_null_v_true_ibis'
        fig_i.savefig(seg_null_hist_save_dir + save_name + '.png')
        fig_i.savefig(seg_null_hist_save_dir + save_name + '.svg')
        plt.close(fig_i)
        ...
        im_name = (' ').join(segment_names[s_i].split('_'))
        plt.hist(seg_null_dev_counts, bins=20, alpha=0.5,
                 color='blue', label='Null Data')
        ...
        plt.tight_layout()
        save_name = ('_').join(
            segment_names[s_i].split(' ')) + '_null_v_true_counts'
        fig_i.savefig(seg_null_hist_save_dir + save_name + '.png')
        fig_i.savefig(seg_null_hist_save_dir + save_name + '.svg')
        plt.close(fig_i)
    ...
    for s_i in range(num_segments):
        ...
        for s_n_i in range(len(seg_null_dev_bout_lens)):
            seg_null_dev_bout_lens_flat.extend(seg_null_dev_bout_lens[s_n_i])
        plt.hist(seg_null_dev_bout_lens_flat, bins=20,
                 color=cmap[s_i], alpha=0.5, label=segment_name + ' null')
    ...
    for s_i in range(num_segments):
        ...
        mean_true = np.mean(seg_true_dev_bout_lens)
        mean_vals.extend([mean_true])
        plt.hist(seg_true_dev_bout_lens, bins=20,
                 color=cmap[s_i], alpha=0.5, label=segment_name + ' distribution')
        plt.axvline(mean_true, color=cmap[s_i], label=segment_name + ' mean')
    plt.xlim((0, max(mean_vals) + 0.25))
    plt.legend()
    plt.xlabel('Deviation Length (s)')
    ...
    for s_i in range(num_segments):
        ...
        mean_true = np.mean(seg_true_dev_ibis)
        mean_vals.extend([mean_true])
        plt.hist(seg_true_dev_ibis, bins=20,
                 color=cmap[s_i], alpha=0.5, label=segment_name + ' distribution')
        plt.axvline(mean_true, color=cmap[s_i], label=segment_name + ' mean')
    #plt.xlim((0,max(mean_vals) + 0.25))
    plt.legend()
    ...
    for s_i in range(num_segments):
        ...
        plt.hist(seg_null_dev_counts, bins=20,
                 color=cmap[s_i], alpha=0.5, label=segment_name + ' null')
        plt.axvline(seg_true_dev_count,
                    color=cmap[s_i], label=segment_name + ' true')
    plt.legend()
