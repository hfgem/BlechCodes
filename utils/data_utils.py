...
class import_multiday_data():
    
    def __init__(self,args):
        day_metadata_dict = args[0]
        self.data_path = day_metadata_dict['dir_name']
        self.hdf5_path = os.path.join(self.data_path, day_metadata_dict['hdf5_dir'])
        dig_in_names = args[1]
        self.get_sampling_rate()
        self.get_spikes()
        self.get_dig_ins(dig_in_names)
    ...
    def get_dig_ins(self, dig_in_names):
        ...
        #Dig In Times
        start_dig_in_times_csv = os.path.join(self.data_path,'start_dig_in_times.csv')
        end_dig_in_times_csv = os.path.join(self.data_path,'end_dig_in_times.csv')
        if os.path.isfile(start_dig_in_times_csv):
            print("\t\tImporting previously saved digital input times")
            start_dig_in_times = []
            with open(start_dig_in_times_csv, 'r') as file:
