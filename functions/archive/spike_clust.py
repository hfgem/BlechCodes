...
def cluster(spikes, peak_indices, e_i, sort_data_dir, axis_labels, type_spike,
    ...
    re_sort, sort_neur_stats_csv, sort_neur_ind_csv, sort_neur_wav_csv = prev_clustered_check(
        sort_data_dir, e_i, type_spike)
    ...
    if re_sort == 'y':
        ...
        if type_spike[0:5] == 'final':
            ...
            clust_num = clust_num_vec[np.where(
                silhouette_scores == np.max(silhouette_scores))[0]][0]
        ...
        neuron_spike_ind, neuron_waveform_ind = import_sorted(
            sort_neur_stats_csv, sort_neur_ind_csv, sort_neur_wav_csv)
...
def prev_clustered_check(sort_data_dir, i, type_spike, re_sort='y'):
    ...
    if os.path.isfile(sort_neur_ind_csv) == False:
        ...
        if re_sort != 'n':
            ...
            print('\t INPUT REQUESTED: The ' + type_spike +
                  ' sorting has been previously performed.')
...
def clust_num_test(e_i, spikes, clust_num, axis_labels, silh_dir, clust_type, wav_type, type_spike):
    ...
    if wav_type == 'full':
        ...
        reduced_spikes = np.concatenate(
            (np.expand_dims(peak_amplitudes, 1), spikes_pca), 1)

    # Cluster data
    if type_spike[0:5] == 'final':
        rand_ind = np.random.randint(
            len(reduced_spikes), size=(len(peak_amplitudes),))
    else:
        rand_ind = np.random.randint(len(reduced_spikes), size=(
            int(np.ceil(len(peak_amplitudes)/3)),))
    ...
    if clust_type == 'kmeans':
        ...
        kmeans = KMeans(n_clusters=clust_num,
                        random_state=np.random.randint(100)).fit(rand_spikes)
        ...
        gmm_fit = gm(n_components=clust_num, random_state=np.random.randint(
            100), max_iter=200).fit(rand_spikes)
        print('\t \t \t  Performing label prediction.')
    ...
    rand_ind = np.random.randint(len(reduced_spikes), size=(int(np.ceil(
        len(peak_amplitudes)/10)),))  # Use less for Silhouette since it takes a long time
    rand_spikes = list(np.array(reduced_spikes)[rand_ind])
    rand_labels = list(np.array(labels)[rand_ind])
...
# 	plt.close(slh_fig)
    ...
    dist = [np.sum((centers[i] - np.array(rand_spikes)
                   [np.where(np.array(rand_labels) == i)[0]])**2, 1) for i in range(clust_num)]
    avg_dist = np.mean([np.mean(dist[i]) for i in range(clust_num)])

    # Plot PCA cluster visualization for final clustering phase only
    if type_spike[0:5] == 'final':
        ...
        clust_fig.savefig(silh_dir + 'projections_clust_count_' +
                          str(clust_num) + '.png', dpi=100)
        plt.close(clust_fig)
...
def spike_clust(spikes, peak_indices, clust_num, i, sort_data_dir, axis_labels,
    ...
    if re_sort == 'y':
        ...
        if clust_type == 'kmeans':
            ...
            kmeans = KMeans(n_clusters=clust_num,
                            random_state=0).fit(rand_spikes)
            print('\t Performing label prediction.')
            labels = kmeans.predict(reduced_spikes)
            centers = kmeans.cluster_centers_
        elif clust_type == 'gmm':
        ...
        for li in range(clust_num):
            ...
            #   c = possible_colors[li])
        ax.legend(loc='center left')
        clust_fig.savefig(sort_neur_type_dir +
                          'cluster_projections.png', dpi=100)
        plt.close(clust_fig)
        ...
        for li in range(clust_num):
            ...
            num_spikes, viol_1_percent, viol_2_percent, avg_fr = calculate_spike_stats(peak_ind,
                                                                                       ...
                                                                                       viol_1, viol_2)
            # Save cluster stats
            clust_stats[li, 0:4] = np.array(
                [num_spikes, viol_1_percent, viol_2_percent, avg_fr])
            ...
            pass_val = (viol_2_percent < viol_2_cutoff) and (
                viol_1_percent < viol_1_cutoff)
            if pass_val == True:
                print("\t \t Cluster " + str(li) +
                      " passed violation cutoffs. Now plotting.")
            else:
                print("\t \t Cluster " + str(li) +
                      " did not pass violation cutoffs.")
        ...
        if (any_good > 0) & (type_spike != 'noise_removal'):  # Automatically skip over bad sorts
            if user_input == 1:  # Ask for user input to select clusters to keep
                ...
                print(
                    "\t Inspect the output visuals of spike clusters, and decide which you'd like to keep.")
                keep_loop = 1
                while keep_loop == 1:
                    keep_any = input(
                        "\t Would you like to keep any of the clusters as spikes (y/n)? ")
                ...
                try:
                    ...
                    while comb_loop == 1:
                        if len(ind_good) > 1:
                            combine_spikes = input(
                                "\t Do any of these spikes come from the same neuron (y/n)? ")
                            if combine_spikes != 'y' and combine_spikes != 'n':
                                ...
                                if combine_spikes == 'y':
                                    while which_comb_loop == 1:
                                        which_together = input(
                                            "\t Which indices belong together [comma separated list]? ").split(',')
                                        try:
                                            together_ind = [
                                                int(which_together[i]) for i in range(len(which_together))]
                                            which_comb.append(together_ind)
                                            if len(ind_good) - len(together_ind) > 1:
                                                cont_loop_2 = 1
                                                while cont_loop_2 == 1:
                                                    continue_statement = input(
                                                        "\t Are there more indices which belong together (y/n)? ")
                                    ...
                                    not_to_combine = np.setdiff1d(
                                        np.array(ind_good), all_to_combine)
                                    ind_good = []
                    ...
                    for ig in ind_good:
                        if np.size(ig) > 1:
                            ...
                            for ind_g in ig:
                                ...
                                peak_ind.extend(
                                    list(np.array(peak_indices)[i_labelled[ind_g]]))
                            ...
                            peak_ind = []
                            peak_ind.extend(
                                list(np.array(peak_indices)[i_labelled[ig]]))
                            neuron_spike_ind.append(peak_ind)
                except:
        ...
        if type_spike != 'noise_removal':
            ...
            for w_i in range(len(neuron_waveform_ind)):
                num_wav = len(os.listdir(save_folder)) + \
                    1  # Index of this set of waveforms
                # Calculate stats
                ...
                num_spikes, viol_1_percent, viol_2_percent, avg_fr = calculate_spike_stats(peak_ind,
                                                                                           ...
                                                                                           viol_1, viol_2)
                # Store cluster stats
                new_clust_stats[w_i, 0:4] = np.array(
                    [num_spikes, viol_1_percent, viol_2_percent, avg_fr])
            ...
            with open(new_sort_neur_stats_csv, 'w') as f:
                ...
                write.writerows([['Number of Spikes', '1 ms Violations',
                                '2 ms Violations', 'Average Firing Rate', 'Good']])
...
def plot_neuron_properties(num_vis, spikes_labelled, peak_ind, sampling_rate,
    ...
    """
    possible_colors = ['b', 'g', 'r', 'c', 'm', 'k', 'y', 'brown', 'pink',
                       'olive', 'gray']  # Colors for plotting different tastant deliveries

    # Select sub-population of spikes to plot as an example
    plot_num_vis = min(num_vis, len(spikes_labelled)-1)
    plot_ind = np.random.randint(0, len(peak_ind), size=(
        plot_num_vis,))  # Pick 500 random waveforms to plot
    ...
    for t_i in range(len(dig_in_times)):  # By tastant
        ...
        PSTH_x_labels = np.arange(-PSTH_left_ms,
                                  PSTH_right_ms, bin_step_size_ms)
        ...
        for b_i in range(len(PSTH_x_labels)):
            ...
            right_ind = int(
                min(round(b_i*bin_step_size + 0.5*bin_size), len(spike_raster[0])))
    ...
    plt.subplot(3, 3, 1)
    spike_bit = spikes_labelled[np.random.randint(
        0, len(spikes_labelled), size=(10000,))]
    ...
    [plt.axvline(segment_times[i], label=segment_names[i], alpha=0.2,
                 c=possible_colors[i]) for i in range(len(segment_names))]
    plt.legend()
    ...
    for d_i in range(len(dig_in_names)):
        [plt.axvline(dig_in_times[d_i][i], c=possible_colors[d_i], alpha=0.2,
                     label=dig_in_names[d_i]) for i in range(len(dig_in_times[d_i]))]
    plt.xlim((min(all_dig_in_times) - sampling_rate,
             max(all_dig_in_times) + sampling_rate))
    ...
    for p_f in range(len(peak_freqs)):
        plt.axvline(peak_freqs[p_f], color=possible_colors[p_f], label=str(
            round(peak_freqs[p_f], 2)))
    ...
    for d_i in range(len(dig_in_times)):
        plt.plot(PSTH_x_labels, PSTH_avg_taste[d_i],
                 c=possible_colors[d_i], label=dig_in_names[d_i])
