...
def run_null_create_parallelized(inputs):
    ...
    fake_spike_times = [random.sample(range(start_t, end_t), len(
        spikes[n_i])) for n_i in range(len(spikes))]
...
def high_bins(inputs):
    ...
    bin_spike_counts = [np.sum(np.sum(bin_spikes[:, b_i:b_i+bin_dt], 1))
                        for b_i in range(segment_end_time-segment_start_time-bin_dt)]
    bin_neur_counts = [np.sum(np.sum(bin_spikes[:, b_i:b_i+bin_dt], 1) > 0)
                       for b_i in range(segment_end_time-segment_start_time-bin_dt)]
    high_neur_bins = dict()
    for t_i in num_thresh:
        high_neur_bins.update(
            {str(t_i): np.sum((np.array(bin_neur_counts) >= t_i).astype('int'))})
    high_spike_bins = dict()
    num_spikes = np.arange(1, max(np.array(bin_spike_counts)), 2)
    for s_i in num_spikes:
        high_spike_bins.update(
            {str(s_i): np.sum((np.array(bin_neur_counts) >= s_i).astype('int'))})
...
def auto_corr(inputs):
    ...
    segment_starts = np.sort(np.random.choice(
        np.arange(seg_length), np.ceil(seg_length/100).astype('int')))
    lag_results = dict()
    for lag_i in range(len(lag_vals)):
        lag = lag_vals[lag_i]
        bin_spike_correlations = [np.corrcoef(bin_spikes[:, b_i], bin_spikes[:, b_i+lag])[
            0, 1] for b_i in segment_starts[segment_starts < seg_length-lag]]
...
def plot_indiv_truexnull(true_x_vals, null_x_vals, true_spike_counts, mean_null_spike_counts,
    ...
    plt.plot(true_x_vals, true_spike_counts,
             label='true bin counts', color='b')
...
# 					plt.text(true_x_vals[p_i],true_spike_counts[p_i]+y_text_buffer,str(percentiles[p_i]))
    ...
    plt.plot(null_x_vals, mean_null_spike_counts,
             color='g', label='null bin counts mean')
    plt.legend()
    ...
    # Log scale
    fig = plt.figure(figsize=(5, 5))
    plt.plot(true_x_vals, np.log(true_spike_counts),
             label='true bin counts', color='b')
# 	if len(percentiles) > 0:
...
# 					plt.text(true_x_vals[p_i],np.log(true_spike_counts[p_i])+y_text_buffer,str(percentiles[p_i]))
    plt.fill_between(null_x_vals,
                     ...
                     (np.log(mean_null_spike_counts+std_null_spike_counts)),
                     alpha=0.4, color='g', label='null bin counts std')
    plt.plot(null_x_vals, np.log(mean_null_spike_counts),
             color='g', label='null bin counts mean')
    plt.legend()
    plt.xlabel(plot_name)
    ...
    # Normalized data
    fig = plt.figure(figsize=(5, 5))
    plt.plot(true_x_vals, true_spike_counts/norm_val,
             label='true bin counts', color='b')
# 	if len(percentiles) > 0:
...
# 					plt.text(true_x_vals[p_i],true_spike_counts[p_i]+y_text_buffer,str(percentiles[p_i]))
    plt.fill_between(null_x_vals,
                     ...
                     (mean_null_spike_counts+std_null_spike_counts)/norm_val,
                     alpha=0.4, color='g', label='null bin counts std')
    plt.plot(null_x_vals, mean_null_spike_counts/norm_val,
             color='g', label='null bin counts mean')
    plt.legend()
...
def plot_all_truexnull(true_x_vals, null_x_vals, true_spike_counts, mean_null_spike_counts,
    ...
    for s_i in range(len(true_x_vals)):
        plt.plot(true_x_vals[s_i], true_spike_counts[s_i],
                 label=segment_names[s_i] + ' true', linestyle='solid', color=colors[s_i])
        plt.fill_between(null_x_vals[s_i],
                         (np.array(
                             mean_null_spike_counts[s_i])-std_null_spike_counts[s_i]),
                         (mean_null_spike_counts[s_i] +
                          std_null_spike_counts[s_i]),
                         alpha=0.4, color=colors[s_i], label=segment_names[s_i] + ' null std')
        plt.plot(null_x_vals[s_i], mean_null_spike_counts[s_i], linestyle='dotted',
                 color=colors[s_i], label=segment_names[s_i] + ' null mean')
    plt.legend()
    plt.xlabel(plot_name)
    plt.ylabel('Number of Instances')
    plt.title(plot_name)
    ...
    fig = plt.figure(figsize=(5, 5))
    for s_i in range(len(true_x_vals)):
        plt.plot(true_x_vals[s_i], np.log(true_spike_counts[s_i]),
                 label=segment_names[s_i] + ' true', linestyle='solid', color=colors[s_i])
        plt.fill_between(null_x_vals[s_i],
                         (np.log(
                         ...
                         np.log(
                             (mean_null_spike_counts[s_i]+std_null_spike_counts[s_i])),
                         alpha=0.4, color=colors[s_i], label=segment_names[s_i] + ' null std')
        plt.plot(null_x_vals[s_i], np.log(mean_null_spike_counts[s_i]),
                 linestyle='dotted', color=colors[s_i], label=segment_names[s_i] + ' null mean')
    plt.legend()
    plt.xlabel(plot_name)
    plt.ylabel('Number of Instances')
    plt.title(plot_name)
    ...
    fig = plt.figure(figsize=(5, 5))
    for s_i in range(len(true_x_vals)):
        plt.plot(true_x_vals[s_i], true_spike_counts[s_i]/norm_val,
                 label=segment_names[s_i] + ' true', linestyle='solid', color=colors[s_i])
        plt.fill_between(null_x_vals[s_i],
                         (np.array(
                             mean_null_spike_counts[s_i])-std_null_spike_counts[s_i])/norm_val,
                         (mean_null_spike_counts[s_i] +
                          std_null_spike_counts[s_i])/norm_val,
                         alpha=0.4, color=colors[s_i], label=segment_names[s_i] + ' null std')
        plt.plot(null_x_vals[s_i], mean_null_spike_counts[s_i]/norm_val,
                 linestyle='dotted', color=colors[s_i], label=segment_names[s_i] + ' null mean')
    plt.legend()
    plt.xlabel(plot_name)
    plt.ylabel('Number of Instances')
