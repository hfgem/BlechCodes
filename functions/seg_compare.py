...
def bin_spike_counts(save_dir,segment_spike_times,segment_names,segment_times):
	...
	segment_pair_count_calculations = KS_test_pipeline(count_save_dir,dist_name,s_i_pairs,segment_counts)
	...
	segment_pair_fr_calculations = KS_test_pipeline(fr_save_dir,dist_name,s_i_pairs,segment_frs)
...
def calculate_fr_distribution(spike_times,bin_size):
	...
	for b_i in range(len(bin_borders)-1):
		bin_frs[b_i] = np.sum(spike_times[:,bin_borders[b_i]:bin_borders[b_i+1]])/bin_size
...
def plot_distributions(results,title,dist_name,save_location):
	...
	plt.savefig(save_location + im_name + '_' + ('_').join(dist_name.split(' ')) + '.png')
	plt.savefig(save_location + im_name + '_' + ('_').join(dist_name.split(' ')) + '.svg')
	plt.close()
...
def KS_test_pipeline(ks_save_dir,dist_name,s_i_pairs,data_dict):
	...
	for pair_i in s_i_pairs:
		...
		pair_results = KS_test_distributions(dist_name,seg_1, seg_2, seg_1_data, seg_2_data, ks_save_dir)
...
def KS_test_distributions(dist_name, name_1, name_2, dict_1, dict_2, fig_save_dir):
	...
	KS_test_plots(dist_name,name_1,name_2,results,bin_size,means,stds,fig_save_dir)
	
	#Plot the KS-Test results as bin size increases without 0 bins included
	KS_test_plots(dist_name,name_1,name_2,results_minus_0,bin_size,means_minus_0,stds_minus_0,fig_save_dir,name_modifier='_no_0')
...
def KS_test_plots(dist_name,name_1,name_2,results,bin_size,means,stds,fig_save_dir,name_modifier=''):
	...
	#CVs subplot
	plt.subplot(3,2,5)
	plt.plot(bin_size,np.array(stds)[:,0]/np.array(means)[:,0],label=name_1)
	plt.plot(bin_size,np.array(stds)[:,1]/np.array(means)[:,1],label=name_2)
	plt.legend()
