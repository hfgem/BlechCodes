...
import os, tqdm, gzip, itertools, json
...
class run_data_null_analysis():
	...
	def get_true_spikes(self,segment_spikes,segment_end_time,segment_start_time):
		bin_spike = np.zeros((self.num_neur,segment_end_time-segment_start_time+1))
		for n_i in range(self.num_neur):
	...
	def get_null_stats(self,segment_start_time,segment_end_time,null_bin_spikes):
		...
		chunk_inds = np.linspace(0,self.num_null,10).astype('int') #chunk it to keep memory usage lower
		results_counts = []
		...
		for c_i in tqdm.tqdm(range(len(chunk_inds)-1)):
			...
			with Pool(processes=4) as pool: # start 4 worker processes
				results_chunk_counts = pool.map(nd.high_bins,zip(null_bin_spike_chunk, 
												 itertools.repeat(segment_start_time), 
												 itertools.repeat(segment_end_time), 
												 itertools.repeat(self.bin_size), 
	...
	def summarize_data(self,true_counts,null_counts):
		#Neuron count
		true_x_vals = np.array([(np.ceil(float(key))).astype('int') for key in true_counts.keys()])
		true_count_array = np.array([true_counts[key] for key in true_counts.keys()])
		null_x_vals = np.array([(np.ceil(float(key))).astype('int') for key in null_counts.keys()])
		mean_null_counts = np.array([np.mean(null_counts[key]) for key in null_counts.keys()])
		std_null_counts = np.array([np.std(null_counts[key]) for key in null_counts.keys()])
		percentiles = [] #Calculate percentile of true data point in null data distribution
		for key in true_counts.keys():
			try:
				percentiles.extend([round(stats.percentileofscore(null_counts[key],true_counts[key]),2)])
			except:
