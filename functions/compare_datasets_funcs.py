	...
	for c_i in range(len(all_combinations_full)):
		...
		for i_1 in range(combo_lengths[0]):
			...
			for i_2 in range(combo_lengths[1]):
				...
				f_all, ax_all = plt.subplots(ncols = combo_lengths[2], figsize=(combo_lengths[2]*5,5))
				f_best, ax_best = plt.subplots(ncols = combo_lengths[2], figsize=(combo_lengths[2]*5,5))
				max_freq = 0
				max_best_freq = 0
				...
				for i_3 in range(combo_lengths[2]):
					...
					for g_n in unique_given_names:
						...
						seg_ind = np.where([corr_data[g_n]['segment_names'][s_i] == seg_name for s_i in range(len(corr_data[g_n]['segment_names']))])[0][0]
						seg_len = corr_data[g_n]['segment_times_reshaped'][seg_ind][1] - corr_data[g_n]['segment_times_reshaped'][seg_ind][0]
						...
						try:
							...
							for t_i, t_name in enumerate(taste_names):
								...
								taste_epoch_mean_corr = np.nanmean(taste_epoch_corr_data,1)
								taste_mean_corrs.append(list(taste_epoch_mean_corr))
							best_taste_per_dev = np.argmax(np.array(taste_mean_corrs),0)
							for t_i, t_name in enumerate(taste_names):
								taste_corr_inds = np.where(best_taste_per_dev == t_i)[0]
								num_all_corr = len(taste_corr_inds)
								num_best_corr = len(np.where(np.array(taste_mean_corrs)[t_i,taste_corr_inds] > 0.5)[0])
					...
					ax_best[i_3].set_xticks(np.arange(1,len(unique_taste_names)+1),unique_taste_names)
				...
				for i_3 in range(combo_lengths[2]):
					...
					for sp_i in list(sig_taste_data.keys()):
						...
						ax_all[i_3].plot([ind_1,ind_2],[sig_height,sig_height],color='k',linestyle='solid')
						ax_all[i_3].plot([ind_1,ind_1],[sig_height-step/2,sig_height+step/2],color='k',linestyle='solid')
						ax_all[i_3].plot([ind_2,ind_2],[sig_height-step/2,sig_height+step/2],color='k',linestyle='solid')
						ax_all[i_3].text(ind_1 + (ind_2-ind_1)/2, sig_height + step/2,'*',horizontalalignment='center',verticalalignment='center')
						sig_height += step
					best_step = max_best_freq/10
					...
					for sp_i in list(best_sig_taste_data.keys()):
						...
						ind_1 = sig_taste_data[sp_i]['ind_1']
						ind_2 = sig_taste_data[sp_i]['ind_2']
						ax_best[i_3].plot([ind_1,ind_2],[best_sig_height,best_sig_height],color='k',linestyle='solid')
						ax_best[i_3].plot([ind_1,ind_1],[best_sig_height-step/2,best_sig_height+step/2],color='k',linestyle='solid')
						ax_best[i_3].plot([ind_2,ind_2],[best_sig_height-step/2,best_sig_height+step/2],color='k',linestyle='solid')
						ax_best[i_3].text(ind_1 + (ind_2-ind_1)/2, best_sig_height + step/2,'*',horizontalalignment='center',verticalalignment='center')
				...
				f_all_name = combo_1.replace(' ','_') + '_' + combo_2.replace(' ','_') + '_all'
				plt.tight_layout()
				...
				f_best_name = combo_1.replace(' ','_') + '_' + combo_2.replace(' ','_') + '_best'
				plt.tight_layout()
		...
		for i_1 in range(combo_lengths[0]):
			...
			for i_2 in range(combo_lengths[1]):
				...
				for i_3 in range(combo_lengths[2]):
					...
					taste_ind = np.where([unique_taste_names[t_i] == taste_name for t_i in range(len(unique_taste_names))])[0][0]
					e_i = att.e_i
					#For each animal collect frequency data for the deviation events which on average are most correlated to a particular taste
					dev_freq_dict = dict()
					for g_n in unique_given_names:
						...
						try:
							...
							for s_i, s_name in enumerate(unique_segment_names):
								seg_ind = np.where([corr_data[g_n]['segment_names'][s_i] == s_name for s_i in range(len(corr_data[g_n]['segment_names']))])[0][0]
								seg_len = corr_data[g_n]['segment_times_reshaped'][seg_ind][1] - corr_data[g_n]['segment_times_reshaped'][seg_ind][0]
								seg_len_s = seg_len/1000
								...
								for t_i, t_name in enumerate(unique_taste_names):
									...
									seg_epoch_mean_corr = np.nanmean(seg_epoch_corr_data,1)
									seg_corr_data_by_taste.append(list(seg_epoch_mean_corr))
								best_taste_per_dev = np.argmax(np.array(seg_corr_data_by_taste),0)
								dev_ind_for_taste = np.where(best_taste_per_dev == taste_ind)[0]
								keep_dev_corrs = np.array(seg_corr_data_by_taste[taste_ind])[dev_ind_for_taste]
								segment_mean_corrs.append(list(keep_dev_corrs))
							for s_i, s_name in enumerate(unique_segment_names):
								num_all_corr = len(segment_mean_corrs[s_i])
								num_best_corr = len(np.where(np.array(segment_mean_corrs[s_i]) > 0.5)[0])
								dev_freq_dict[g_n][s_name] = dict()
								dev_freq_dict[g_n][s_name]['all'] = num_all_corr/segment_lengths[s_i]
								dev_freq_dict[g_n][s_name]['best'] = num_best_corr/segment_lengths[s_i]
						except:
							print("No data.")
					#For each taste create boxplots of the rate distributions
					ax_all[i_3].axhline(0,label='_',alpha=0.2,color='k',linestyle='dashed')
					...
					for s_i, s_name in enumerate(unique_segment_names):
						...
						for g_n in unique_given_names:
							try:
								segment_rates.extend([dev_freq_dict[g_n][s_name]['all']])
							except:
								print("Skipping dataset.")
					...
					ax_all[i_3].set_xlabel(i3_label)
					ax_all[i_3].set_ylabel('Frequency (Hz)')
					ax_best[i_3].set_xticks(np.arange(1,len(unique_segment_names)+1),unique_segment_names)
					ax_best[i_3].set_xlabel(i3_label)
					ax_best[i_3].set_ylabel('Frequency (Hz)')
					#Test pairwise significance #TTEST
				...
				for i_3 in range(combo_lengths[2]):
					...
					for m_i in range(len(mean_diff_collection)):
						...
						if len(points)>0:
							...
							ax_mean_diff[0,i_3].boxplot([points],positions=[m_i+1],sym='',medianprops=dict(linestyle='-',color='blue'),showcaps=True,showbox=True)
					...
					for sp_i in list(sig_taste_data.keys()):
						try: #Pair data
							...
							for sig_plot_ind in dist_sig_data:
								ax_mean_diff[0,i_3].text(sig_plot_ind, sig_height,'*',horizontalalignment='center',verticalalignment='center')
							sig_height += step
					#Adjust y-limits
				...
				for i_3 in range(combo_lengths[2]):
					...
					for m_i in range(len(mean_diff_collection)):
						...
						if len(points)>0:
							...
							ax_mean_diff[i_3].boxplot([points],positions=[m_i+1],sym='',medianprops=dict(linestyle='-',color='blue'),showcaps=True,showbox=True)
						points_boxplot.append(list(points))
					if len(points_boxplot) > 0:
						if max_mean_diff < max_mean_diff_i:
							max_mean_diff = max_mean_diff_i
				...
				for i_3 in range(combo_lengths[2]):
					...
					for sp_i in list(sig_taste_data.keys()):
						try: #Pair data
							...
							dist_sig_data = sig_taste_data[sp_i]
							for sig_plot_ind in dist_sig_data:
								ax_mean_diff[i_3].text(sig_plot_ind, sig_height,'*',horizontalalignment='center',verticalalignment='center')
							sig_height += step
					#Adjust y-limits
