	...
	for t_i in range(num_tastes):
		...
		for n_i in tqdm.tqdm(range(num_neur)):
			...
			for d_i in range(num_deliv):
				...
				peak_likelihoods = [] #Relative - each time divided by the max likelihood of that iteration
					...
					for b_i in range(len(sub_bins)-1):
						...
						if (sub_bin_len > cp_bin) & (sub_bin_spikes > 0):
							...
							for time_i in np.arange(sub_bins[b_i]+cp_bin,sub_bins[b_i+1]):
								...
								N_2 = np.sum(deliv_bin[time_i+1:sub_bins[b_i+1]])
								T_1 = time_i - sub_bins[b_i]
								T_2 = sub_bins[b_i+1] - time_i
								cp_likelihood_bin[time_i] = (((N_1/T_1)**N_1)*((N_2/T_2)**N_2))/(((N_1+N_2)/(T_1+T_2))**(N_1+N_2))
							bin_peak_inds = find_peaks(cp_likelihood_bin[cp_bin:],distance=cp_bin)[0]
							bin_peak_inds = bin_peak_inds[bin_peak_inds > before_taste] + cp_bin
							if len(bin_peak_inds) > 0:
								...
								if iter_i > 0:
									for spi in range(len(sorted_peak_ind)):
										...
										max_peak_likelihood = cp_likelihood_bin[max_peak_ind]#/np.max(cp_likelihood_bin)
										close_existing_peaks = np.where(np.abs(np.array(peak_inds) - max_peak_ind) < cp_bin)[0]
...
def plot_cp_rasters_pop(deliv_bin,deliv_cp,d_i,before_taste,dig_in_name,taste_cp_save_dir):
	...
	deliv_st = [np.where(deliv_bin[n_i,:] > 0)[0] - before_taste for n_i in range(num_neur)]
